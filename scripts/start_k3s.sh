...
K3S_VERSION=${K3S_VERSION:-"v1.22.9-k3s1"}
...
## Waiting for start
while [ "${k3s_ready}" != "pong" ] && [ "${k3s_running}" != "false" ]; do
  sleep 5
  k3s_ready=$(${CURL_BIN} -sk ${K3S_URL}/ping || echo starting rancher k3s cluster)
  k3s_running=$(${DOCKER_BIN} inspect ${K3S_SERVER} | ${JQ_BIN}  -r .[0].State.Running)
done
if [ "${k3s_running}" == "false" ]; then
  echo "==> Failed to run k3s server"
  ${DOCKER_BIN} logs ${K3S_SERVER}
fi

echo "==> Copying kubeconfig to ${K3S_KUBECONFIG}"
